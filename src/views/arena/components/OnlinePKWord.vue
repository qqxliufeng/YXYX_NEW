...
import AddWordArena from './AddWordArena.vue'
import ArenaWordDetails from './ArenaWordDetails.vue'
export default {
  name: 'OnlinePKWord',
  components: {
    AddWordArena,
  ...
  methods: {
    ...
    onAdd() {
      ...
      this.$refs.addWordArena.showDialog()
    },
    showDetails(item) {
      if (!this.checkButtonPermission('arena_details')) {
        return
      }
      this.$refs.arenaWordDetails.showDialog()
      this.arenaItem = item
    }
  }
}
